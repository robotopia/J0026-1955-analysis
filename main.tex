...
Pulsars, or rapidly rotating neutron stars, are the sites of some of the highest energy physical processes in the universe, owing to the intense gravitational and magnetic fields that surround them.
Discovered first via their lighthouse-like beam of radio emission \citep{Hewish1968}, which we detect as a series of regularly spaced pulses, it was quickly realised that the physics governing the relativistic plasma that generates this beam was not well understood---a state of affairs that persists to the present day.
The uniqueness of each pulsar's emission signature provides a wealth of data that can be used to test candidate emission mechanisms, and it has been proven again and again over the past several decades that in-depth studies of individual pulsars can be just as useful for uncovering vital clues for understanding the population as a whole.
This, in part, motivates efforts to find new pulsars via large scale pulsar surveys, which continue to be conducted up to the present day.

One of the most recent such surveys is currently under way at the Murchison Widefield Array \citep[MWA;][]{Tingay2013}, a latest-generation aperture array telescope located in Western Australia.
...
This strategy quickly paid off with the discovery of PSR \psrnick{}, a low-luminosity, high Galactic latitude pulsar, reported in \citep{Swainston2021}.
...
PSR \psr{} was discovered in one of the observations taken as part of the SMART survey (Obs ID 1226062160).
...
\tablewidth{0pt}
\tablecaption{Observations of PSR \psr{} \label{tab:observations}}
\tablehead{
\colhead{MJD} & \colhead{Obs ID} & \colhead{Number of rotations}
}
\startdata
...
The brightness of \psr{}'s single pulses begs the question of how it avoided detection in previous, more sensitive pulsar surveys covering the same sky (albeit at higher frequencies).
...
$P_3$ and the the drift rate are defined continuously over all pulse numbers (even fractional ones) by virtue of the fact that the drift bands are modelled as continuous functions on the pulse stack, although it is understood that these quantities are strictly only defined at integer pulse numbers.
...
The combination of these behaviors in one pulsar is rare, and a comparison of \psr{} with other pulsars showing similar behaviors is given in Table \ref{tab:comparison}.
...
\startdata
\psr{} & 77 & (A) 20-50 & $\gtrsim 100$ & This work \\
B0031$-$07 & 45 & $\sim[12,7,6]$ & $\gtrsim 50$ & \citet{Joshi2000,McSweeney2017,McSweeney2019a} \\
...
It is bright enough to see in single pulses at MWA frequencies, but it is suspected to have been missed by previous Southern Hemisphere surveys by virtue of its long null sequences which can in some cases can exceed 20 minutes.
Its slow period and intermittent nature mean that long duration observations are required to collect a sufficient number of complete burst sequences to do robust statistical analysis on their properties.

However, even with the relatively few observations presently at our disposal, \psr{} clearly exhibits many interesting single pulse behaviors that are deserving of further study.
