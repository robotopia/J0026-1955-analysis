...
Sub-pulse drifting is easily detected when viewing the pulses in a stack (Fig \ref{fig:pulsestacks}), in which associated sets of sub-pulses form diagonal ``drift bands'' that stretch across the on-pulse window.
The drift \textit{rate} is defined as the reciprocal of the slope of the drift bands, often expressed in units of degrees (of rotation) per pulse period ($^\circ/P$).
...
Within this context, pulsars with either multiple (stable) drift rates, or with drift rates that change slowly over time, deserve special consideration.
Pulsars with multiple drift rates \citep{Hankins1987} usually show very rapid transitions ($\lesssim 1\,P$) when switching between them, and pulse sequences with similar drift rates are assigned to the same drifting \textit{mode}.
In practice, the modes are more easily characterised by the vertical spacing between the drift bands, a quantity traditionally called $P_3$.

The other type of sub-pulse drifting anomaly is where the drift rate varies relatively slowly.
This can happen in a variety of ways.
\citet{Lyne1983} linked the variability of the drift rates of B0809$+$74 and B0818$-$13 to the null sequence preceding the onset of a drift mode, showing that the drift band phase is ``remembered'' across the null.
...
These ``variations'' from stable drifting present a natural challenge to the carousel model, whose most basic formulation predicts only a single stable drifting mode, due to the expected stability of the underlying electric and magnetic fields that generate the carousel motion.
...
This idea has been invoked to explain the multiple drift rates of B1918$+$19 \citep{Rankin2013}, B1819$-$22 \citep{Joshi2017}, and B0031$-$07 \citep{McSweeney2019a}.
Aliasing can also ``amplify'' the variability of the drift rate if the ratio of stellar rotation and carousel rotation is very close to the ratio of small integers.
That is, a small change in the magnetospheric conditions near the stellar surface (where the sparks reside) can appear to the observer as a relatively large of the drift rate.
\citet{VanLeeuwen2012} elucidated the connection between the accelerating potential within the polar gap and the carousel behaviour, and found that the variability of B0826$-$34's drift rate can be explained in this way.

Despite the continued success of the carousel model to explain the various observed counterexamples of stable drifting, there remain many open questions, such as what sets the number of sparks, and what drives the transition between different modes.
Exploration of these questions is hampered by the relatively small number of pulsars amenable to this kind of analysis, and is compounded by the difficulty of deriving the carousel configuration from the observed drift rate, for which there are oftentimes many degenerate solutions.
The discovery, therefore, of another bright sub-pulse drifting pulsar is a very welcome addition, particularly as this new pulsar appears to exhibit many of the rare drifting behaviors just discussed.
Section \S\ref{sec:observations} describes the observation in which the pulsar was originally found, as well a small number of archival MWA observations in which it was subsequently detected during follow up of the original candidate.
...
All four observations (summarised in Table \ref{tab:observations}) were made in the frequency range $138.88$-$169.6\,$MHz.
...
\tablehead{
\colhead{MJD} & \colhead{Obs ID} & \colhead{Number of pulses}
}
\startdata
58434 & 1226062160 & 3759 \\
58991 & 1274143152 & 914  \\
59003 & 1275178816 & 914  \\
59094 & 1283104232 & 1374
\enddata
\end{deluxetable}
...
Profiles of each detection, as well as a stacked profile incorporating the data from all four observations, is shown in Fig \ref{fig:profiles}.
Subsequent analysis of the pulse stacks is described in \S\ref{sec:analysis}.
\begin{figure}[!th]
    \centering
    \includegraphics[width=\linewidth]{profiles.png}
    \caption{Mean profiles of the four observations with detections, and the total mean profile (top panel) including all four observations. The profiles show a barely resolved double peak structure.}
    \label{fig:profiles}
\end{figure}
...
The discovery observation (Obs ID 1226062160) was taken when the MWA was in its \textit{compact} configuration \citep[see][]{Wayth2018}, giving an effective tied-array beam size of $\sim$ 30$^\prime$ at the central frequency of $155\,$MHz.
...
The other three observations in which the pulsar has been detected were taken when the MWA was in its extended configuration, giving a tied-array beam size of $\sim$ 3$^\prime$ at $155\,$MHz.
...
Although it is known that the ionosphere is capable of shifting the apparent position of sources by an appreciable fraction of the tied-array beam---and that these offsets are not necessarily corrected by the calibration process when the calibration observations are separated from the target observation spatially and temporally \citetext{Swainston et al. 2021, under review}---the localization afforded by the extended array observations was sufficiently precise (a few arcminutes) to warrant follow-up with a high-resolution imaging telescope.
Accordingly, images made with Band 3 ($300$-$500\,$MHz) data from the upgraded Giant Metrewave Radio Telescope \citep[uGMRT;][]{Gupta2017} obtained as director discretionary time enabled us to confirm the pulsar's position to an accuracy of [Ramesh: TODO].
The final position, consistent with both the uGMRT imaging and the MWA extended configuration observations, is RA 00:26:37.50, Dec -19:56:15.04 [UPDATE THIS WITH LATEST NUMBERS].
...
The values thus derived were $P = 1.306150 \pm 0.000005\,$s and $\text{DM} = \psrdm\,$\dmunit.
...
Nevertheless, the most economical description of the observed drifting behaviour is that it consists primarily of two modes, which we label A and B.
...
Mode A is characterised by slowly evolving $P_3$ values that can range between $\sim$ 20-55\,$P$, and drift rates in the range $-0.3$ to $-0.7^\circ/P$.
Most commonly, $P_3$ is seen to \textit{increase} over time, as the examples in Fig. \ref{fig:pulsestacks} show.
However, there is at least one clear instance in MJD 59094 observation where $P_3$ decreases from approximately $35$ to $24\,P$ over the course of 170 rotations.
Mode A sequences can last anywhere from a a few tens to a few hundreds of pulses, although the identification of shorter sequences as Mode A sequences relies on measuring the drift rate, since in such cases the sequence is not long enough to measure $P_3$ directly.

The distinction between the subcategories A1 and A2 is based primarily on the level of organisation of the drifting behaviour.
...
These interruptions can last between $\sim$ 5-10 pulses, but afterwards the drift bands reappear at the rotation phase that one would expect if the interruptions had not occurred.
...
\begin{equation}
    ...
    \label{eqn:exp_dr}
\end{equation}
where $D_0$ is the drift rate at the beginning of the drift sequence; $D_f$ is the asymptotic drift rate; $p$ is the number of pulses since the onset of the drift sequence; and $\tau_r$ is the drift rate relaxation time (in units of the rotation period).
...
The threshold is chosen to be the minimum pixel value for which no sub-pulses are identified in the off-pulse region.
Each sub-pulse in the drift sequence is then assigned a drift band number, and fitted to the following functional form of the sub-pulse phases (using Python's \texttt{curve\_fit} method).
This functions is obtained by considering the drift rate as the rate of change of sub-pulse phase with pulse number, and integrating Eq. \eqref{eqn:exp_dr}:
\begin{equation}
    \varphi = \tau_r D_0 \left(1 - e^{-p/\tau_r}\right) + D_f p + (\varphi_0 + P_2 d).
    \label{eqn:exp_phase}
\end{equation}
where $\varphi$ is the phase of a sub-pulse; $\varphi_0$ is an initial reference phase; and $d$ is the (integer) drift band number.
In total, this model has five free parameters, $D_0$, $D_f$, $\tau_r$, $\varphi_0$, and $P_2$, of which the expression $\varphi_0 + P_2 d$ forms the integration constant.
...
Given this, the primary utility of the empirical model is its ability to predict other measurable quantities such as $P_2$, $P_3$, and the drift rate, which are less sensitive to the the precise functional form used.
...
$P_3$ and the the drift rate are defined continuously over all pulse numbers (even fractional ones) by virtue of the fact that the drift bands are modelled as continuous functions on the pulse stack, although it is understood that these quantities are only strictly only defined at integer pulse numbers.
Eq. \eqref{eqn:exp_dr} gives the drift rate, and one can derive $P_3 = P_2/|D|$ directly therefrom.
...
\begin{figure}[!th]
    ...
    \caption{A mode A2 sequence from the MJD 58434 observation, with fitted model drift band overlaid in white. The ``interruptions'' from the otherwise good fit can be seen most clearly around pulse numbers $\sim450$, $\sim490$, and $\sim620$.}
...
\begin{figure}[t]
    ...
    \label{fig:lrfs}
\end{figure}
Despite the changing $P_3$ and the relative disorganization of the drift rate, the LRFS and 2DFS show surprisingly high-Q features at $P_3/P \approx 33 \pm 4$ and $P_2 \approx 16^\circ \pm 2^\circ$, which are consistent with the fitted model values of $P_3/P$ changing from $\sim50$ to $27$ throughout the sequence, and $P_2 = 13.8^\circ \pm 0.3^\circ$.
The relative lack of power in the LRFS (bottom panel) near phase $\sim2^\circ$ is likely related to the interruptions to the organization of the drift bands that occurs at that phase, discussed in the previous section.

\section{Discussion}
\label{sec:discussion}

PSR \psr{} is a clear example of a pulsar that exhibits sub-pulse drifting, and that the drifting behaviour is qualitatively similar to other pulsars of the same class.
...
The most remarkable feature of \psr{}'s drifting behaviour is how the drift rate evolves over time, exhibiting both rapid changes (indicating the presence of multiple, distinct drift modes) as well as gradual, ``intra-mode'' changes.
The combination of these behaviours in one pulsar is rare, and a comparison of \psr{} with other pulsars showing similar behaviors is given in Table \ref{tab:comparison}.
...
\startdata
\psr{} & 77 & (A) 20-50 & Large & This work \\
B0031$-$07 & 45 & $\sim[12,7,6]$ & Large & \citet{Joshi2000,McSweeney2017,McSweeney2019a} \\
...
Among the observations of \psr{}, only one mode A1 drift sequence was found whose fitted relaxation time of $\tau_r = \sim31\,P$ was significantly shorter than the drift sequence itself (285 pulses).
...
Modelling the drift sequences in this way opens up new avenues for testing for relationships between the drift rate, the drift decay rate, the duration of the preceding drift (or null) sequence, the possible phase connection between consecutive drift modes (or across nulls), and others.
For example, in many instances the driftbands appear to connect smoothly across mode switches between modes A and B, which, if true, has implications for the mechanisms by which mode transitions occur in the context of the carousel model.
...
We have reported the first obesrvations of PSR \psr{}, the second pulsar discovered as part of the MWA's SMART pulsar survey.
It is bright enough to see in single pulses at MWA frequencies, but it suspected to have been missed by previous Southern Hemisphere surveys by virtue of its long null sequences which can in some cases can exceed 10 minutes.
Its slow period and intermittent nature mean that long duration observations are required to collect enough a sufficient number of complete burst sequences to do robust statistical analysis on their properties.

However, even with the relatively few observations presently at our disposal, \psr{} clearly exhibits many interesting single pulse behaviours that are deserving of further study.
Among these, the most prominent is the manner in which the drift rate changes slowly throughout drift sequences (a property observed in only a small number of other sub-pulse drifting pulsars), as well is the interplay between the two drift mode classes identified here (modes A and B) and the intervening null sequences.
These properties make the interpretation of this pulsar's drifting behaviour particularly interesting in the context of the carousel model.
