...
This strategy quickly paid off with the discovery of PSR \psrnick{}, a low-luminosity, high Galactic latitude pulsar, reported in \citet{Swainston2021}.

\textbf{In this paper, we report the MWA's detection and analysis of PSR \psr{}, which emerged as a candidate as part of the same initial search strategy.}
...
Sub-pulse drifting is easily detected when viewing the pulses in a stack (Fig \ref{fig:pulsestacks}), in which associated sets of sub-pulses form diagonal \textit{drift bands} that stretch across the on-pulse window.
...
Section \S\ref{sec:observations} describes the observation in which the pulsar was originally found and the small number of archival MWA observations in which it was subsequently detected during follow up of the original candidate.
...
After determining the best dispersion measure (DM) and period (see \S\ref{sec:period}), de-dispersion and folding were performed with DSPSR \citep{VanStraten2011b} and PSRCHIVE \citep{Hotan2004} to form single pulse archives, from which Stokes I pulse stacks were created.
...
\textbf{Although this pulsar emerged as a candidate in the shallow pass of SMART, it was subsequently noted to have been reported in 2018 as a candidate in the Green Bank Northern Celestial Cap (GBNCC) pulsar survey \citep{Stovall2014}.
...
These detections confirm the candidate as a true pulsar.}

Follow-up of the initial MWA detection with the full 80 minutes, as well as in other archival MWA observations, revealed that the pulsar has a very large nulling fraction ($\sim$ \nullfraction{})\footnote{The nulling fraction estimate comes only from analysis of the MWA data.}, with some nulling sequences potentially exceeding $20\,$min (i.e.\ the longest archival MWA observation in which it was \textit{not} detected with a periodicity search).
A full analysis of the nulling properties is presented in Section \S\ref{sec:nulling} below.
\textbf{Despite the fact that \psr{} was serendipitously discovered in the first ten minutes, it may well be} the harbinger for a population of relatively bright, intermittent pulsars which the SMART survey, with its $80$-minute dwell times, is well-suited to detect.
...
Although it is known that the ionosphere is capable of shifting the apparent position of sources by an appreciable fraction of the tied-array beam---and that these offsets are not necessarily corrected by the calibration process when the calibration observations are separated from the target observation, both spatially and temporally \citetext{Swainston et al., submitted}---the localization afforded by the extended array observations was sufficiently precise (a few arcminutes) to warrant follow-up with a high-resolution imaging telescope.
Accordingly, images made with Band 3 ($300$-$500\,$MHz) data from the upgraded Giant Metrewave Radio Telescope \citep[uGMRT;][]{Gupta2017}, obtained as director discretionary time, enabled us to confirm the pulsar's position to an accuracy of $\sim1^\prime$.
...
Planned follow-up observations (and a more complete search through archival MWA data) will allow us to place much stronger constraints on the nulling fraction, and will be reported in a future publication.
}
...
\textbf{We have reported the independent discovery and first MWA observations of PSR \psr{} in the SMART pulsar survey.
It is bright enough to see in single pulses at MWA frequencies, but contains long null sequences} which in some cases can exceed 20 minutes\footnote{The presence of intermittent, mode B2 pulses was not searched via, e.g.\ single pulse searches.}.
Its slow period and intermittent nature mean that long duration observations are required to collect a sufficient number of complete burst sequences to undertake robust statistical analysis on their properties.
...
We would also like to thank Alex McEwen for making the the original discovery of this pulsar in the GBNCC survey.
