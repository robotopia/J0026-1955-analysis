...
{\bf
Pulsars that both show complicated drifting behavior and which are bright enough for single pulse analysis, are relatively rare.
}
...
{\bf The MWA is located in a very radio quiet location, and no significant RFI was found in the beamformed data.}
...
\begin{figure}[!th]
    ...
    \caption{{\bf Peak-normalised} mean profiles of the four observations with detections, and the total mean profile (top panel) including all four observations, with a time resolution of $\sim 1.3\,$ms. The profiles show a barely resolved double peak structure.}
    \label{fig:profiles}
\end{figure}
...
Localization within each of these observations followed the identical procedure as used in \citet{Swainston2021}, i.e.\ gridding the candidate positions with overlapping tied-array beams and using signal-to-noise and the estimated beam shape to produce a likelihood map of the source position.
